...
def test_handle_line_Client_unit_state_damaging_unit():
    ...
    if "Client unit state damaging unit" in line:
        ...
        attackingUnit = int(re.search("(Attacker-\\(EntityHandle:\\d*)", line).group(0).split(':')[1])
        damageAmount = int(re.search("(Damage Amount-\\d*)", line).group(0).split('-')[1])
...
def test_damage_type_parser():
    ...
    damage_type = damage_type.removeprefix(
